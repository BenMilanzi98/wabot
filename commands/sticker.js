...
if(cmd_user.isMe||parseInt(data_level)>=2){
    ...
    if (quotedMsg.hasMedia) {
        ...
        await client.sendMessage( idk, new MessageMedia(attachmentData.mimetype, attachmentData.data, attachmentData.filename), { sendMediaAsSticker: true ,stickerAuthor:"Ben"});
    } else { 
        await client.sendMessage(idk, `🙇‍♂️ *Error*\n\n` + "```No image found to make a Sticker```");
    }
}
else{
    await msg.reply("*Ben Has Revoked Your Access To This Command!*");
}
}else{
