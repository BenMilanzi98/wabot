      ...
      coll.find(query).toArray(function(err, result) {
        ...
        for(let i=size-1;i>=0;i--){
          try{
            ...
            if(user_id_string!="265884032588"){
            var tag_replaced_msg=extracted_array[i].message.replace(user_id_string,extracted_array[i].TaggedOne);
            }
            else if(user_id_string=="265884032588"){
              var tag_replaced_msg=extracted_array[i].message.replace(user_id_string, "Ben");
            }
            ...
            console.log(error);
            overall_msg="_Error, Please Notify Ben About This_";
            break;
          }
